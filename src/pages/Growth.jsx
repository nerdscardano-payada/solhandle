...
export default function Growth() {
  ...
  return <main className="min-h-screen bg-slate-950 text-white"><div className="mx-auto min-h-screen max-w-7xl border-x border-white/10"><Header/><section className="mx-auto max-w-5xl px-5 py-12 sm:py-20">
    ...
    {error ? <div role="alert" className="mt-10 text-rose-300">{error}</div> : !data ? <p className="mt-10 text-cyan-200">Geverifieerde voortgang laden…</p> : !c ? <div className="mt-10 rounded-2xl border border-white/10 bg-slate-900/60 p-6 text-slate-300">De eerste cyclus wacht op een geverifieerde mainnet-meting.</div> : <>
      <div className="mt-10 rounded-2xl border border-cyan-300/25 bg-slate-900/70 p-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs uppercase tracking-widest text-cyan-300">Cyclus {c.cycle_number}</p><p className="mt-3 text-5xl font-semibold">{liveProgress}% <span className="text-lg text-slate-400">indicatief</span></p></div><span className="text-sm text-slate-400">{next ? `Volgende officiële beloning bij ${next}%` : 'Alle officiële mijlpalen bereikt'}</span></div><div role="progressbar" aria-label="Indicatieve totale groei" aria-valuemin={0} aria-valuemax={100} aria-valuenow={liveProgress} className="mt-6 h-3 overflow-hidden rounded-full bg-slate-800"><div className="h-full rounded-full bg-gradient-to-r from-emerald-300 via-cyan-300 to-violet-400" style={{ width: `${liveProgress}%` }}/></div><p className="mt-3 text-xs text-slate-400">40% nieuwe mints + 40% nieuwe gekwalificeerde houders + 20% market cap. De balk beweegt mee met de live marktwaarde; {officialProgress === null ? 'de officiële meting van het nieuwe doel volgt bij de volgende uurlijkse update' : `de officiële, uurlijks gemeten voortgang is ${officialProgress}% en bepaalt welke mijlpalen bereikt zijn`}. 100% vereist alle drie.</p></div>
      <div className="mt-4 grid gap-4 md:grid-cols-3"><GrowthMeter label="Nieuwe SolHandles · 40%" current={c.handles_now} baseline={c.handles_start} target={c.handles_target} percentage={handles}/><GrowthMeter label="Nieuwe houders · 40%" current={c.holders_now} baseline={c.holders_start} target={c.holders_target} percentage={holders}/><GrowthPriceMeter current={displayedCap} target={targetCap} percentage={capProgress} source={marketCap === null ? 'snapshot' : marketStatus === 'unavailable' ? 'stale' : 'live'}/></div>
      <p className="mt-4 text-xs leading-relaxed text-slate-400">Doel deze cyclus: +{c.handles_target} betaalde mainnet-mints, +{c.holders_target} wallets met minimaal {data.minimum_balance.toLocaleString()} $HANDLE gedurende 24 uur en een market cap van $30k. Bestaande houders en handles vormen de nulmeting. Market cap (niet tokenprijs) uit de meest liquide officiële Solana-pool op DEX Screener; de uurlijkse meting kan afwijken van de actuele marktwaarde. Start {new Date(c.started_at).toLocaleDateString('nl-BE')}. Groei laatst gemeten {new Date(c.last_checked_at).toLocaleString('nl-BE')}. Market cap laatst geverifieerd {c.market_cap_checked_at ? new Date(c.market_cap_checked_at).toLocaleString('nl-BE') : 'nog niet gemeten'}.</p>
      <GrowthHowTo minimumBalance={data.minimum_balance}/><GrowthMilestones progress={officialProgress ?? 0} cycle={c.cycle_number}/>
