...
export default function Growth() {
  ...
  const next = [25, 50, 75, 100].find(n => n > (c?.max_progress || progress));
  return <main className="min-h-screen bg-slate-950 text-white"><div className="mx-auto min-h-screen max-w-7xl border-x border-white/10"><Header/><section className="mx-auto max-w-5xl px-5 py-12 sm:py-20">
    ...
    {error ? <div role="alert" className="mt-10 text-rose-300">{error}</div> : !data ? <p className="mt-10 text-cyan-200">Geverifieerde voortgang laden…</p> : !c ? <div className="mt-10 rounded-2xl border border-white/10 bg-slate-900/60 p-6 text-slate-300">De eerste cyclus wacht op een geverifieerde mainnet-meting.</div> : <>
      <div className="mt-10 rounded-2xl border border-cyan-300/25 bg-slate-900/70 p-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs uppercase tracking-widest text-cyan-300">Cyclus {c.cycle_number}</p><p className="mt-3 text-5xl font-semibold">{progress}% <span className="text-lg text-slate-400">bereikt</span></p></div><span className="text-sm text-slate-400">{next ? `Volgende beloning bij ${next}%` : 'Alle mijlpalen bereikt'}</span></div><div role="progressbar" aria-label="Totale groei" aria-valuemin={0} aria-valuemax={100} aria-valuenow={progress} className="mt-6 h-3 overflow-hidden rounded-full bg-slate-800"><div className="h-full rounded-full bg-gradient-to-r from-emerald-300 via-cyan-300 to-violet-400" style={{ width: `${progress}%` }}/></div><p className="mt-3 text-xs text-slate-400">40% nieuwe mints + 40% nieuwe gekwalificeerde houders + 20% tokenprijs. Elk onderdeel heeft zijn eigen doel; 100% vereist alle drie.</p></div>
      <div className="mt-4 grid gap-4 md:grid-cols-3"><GrowthMeter label="Nieuwe SolHandles · 40%" current={c.handles_now} baseline={c.handles_start} target={c.handles_target} percentage={handles}/><GrowthMeter label="Nieuwe houders · 40%" current={c.holders_now} baseline={c.holders_start} target={c.holders_target} percentage={holders}/><GrowthPriceMeter start={c.price_start_usd} current={c.price_now_usd} targetPercent={c.price_target_percent || 20} percentage={price}/></div>
      <p className="mt-4 text-xs leading-relaxed text-slate-400">Doel deze cyclus: +{c.handles_target} betaalde mainnet-mints, +{c.holders_target} wallets met minimaal {data.minimum_balance.toLocaleString()} $HANDLE gedurende 24 uur en +{c.price_target_percent || 20}% tokenprijs ten opzichte van de eerste beschikbare prijsmeting in deze cyclus. Bestaande houders en handles vormen de nulmeting. Tokenprijs uit de meest liquide officiële Solana-pool op DEX Screener; de uurlijkse meting kan afwijken van de actuele koers. Start {new Date(c.started_at).toLocaleDateString('nl-BE')}. Laatst gemeten {new Date(c.last_checked_at).toLocaleString('nl-BE')}.</p>
      <GrowthHowTo minimumBalance={data.minimum_balance}/><GrowthMilestones progress={c.max_progress || progress} cycle={c.cycle_number}/>
