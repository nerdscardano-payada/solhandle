...
export default function Admin() {
  ...
  return <main className="min-h-screen bg-[#050811] text-white"><div className="mx-auto min-h-screen max-w-7xl border-x border-white/10"><Header/><section className="px-5 py-12 md:px-9"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm text-cyan-300">Protocol operations</p><h1 className="mt-2 text-4xl font-semibold">Developer dashboard</h1><p className="mt-3 text-slate-400">Treasury, income and on-chain claims for SolHandle.</p></div><button onClick={loadDashboard} disabled={loading} className="inline-flex items-center gap-2 rounded-lg border border-cyan-300/40 px-4 py-2 text-sm text-cyan-200 disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />Refresh</button></div>{data ? <><AdminMetrics data={data}/><AdminMintTable mints={data.recentMints}/><p className="mt-4 text-xs text-slate-500">Indexer last synced: {data.lastSync ? new Date(data.lastSync).toLocaleString() : "Not available"}</p></> : <div className="card-glow mt-8 text-slate-400">Loading protocol data…</div>}</section></div></main>;
}