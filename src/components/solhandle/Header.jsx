...
import UpcomingMenu from "@/components/solhandle/UpcomingMenu";
...
export default function Header({ onConnected }) {
  ...
  return <header className="relative flex items-center justify-between border-b border-white/10 px-5 py-4 lg:px-9"><Brand /><nav className="hidden items-center gap-5 xl:flex"><DiscoverMenu />{links.map(([label, path]) => <NavLink key={path} to={path} className={linkClass}>{label}</NavLink>)}<UpcomingMenu /><DevelopersMenu /><MoreMenu /></nav><div className="flex items-center gap-3"><HeaderExternalLinks/><span className="hidden text-xs text-emerald-300 sm:inline">MAINNET</span><MarketplaceNotificationBell wallet={wallet} bids={notifications.bids} sales={notifications.sales} onRefresh={notifications.refresh}/><WalletButton onConnected={onConnected} /><button type="button" onClick={() => setMenuOpen((open) => !open)} className="text-slate-300 xl:hidden" aria-label="Toggle navigation" aria-expanded={menuOpen}>{menuOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}</button></div>{menuOpen && <nav className="absolute inset-x-0 top-full z-50 border-b border-white/10 bg-slate-950 px-5 py-4 shadow-xl xl:hidden"><p className="px-4 pb-1 text-xs font-semibold uppercase tracking-wider text-slate-500">Discover</p>{discoverLinks.map(([label, path]) => <NavLink key={path} to={path} onClick={() => setMenuOpen(false)} className={({ isActive }) => `block border-l-2 py-2 pl-7 text-sm ${isActive ? "border-emerald-300 text-white" : "border-transparent text-slate-400"}`}>{label}</NavLink>)}{links.map(([label, path]) => <NavLink key={path} to={path} onClick={() => setMenuOpen(false)} className={({ isActive }) => `block border-l-2 py-3 pl-4 text-sm ${isActive ? "border-emerald-300 text-white" : "border-transparent text-slate-400"}`}>{label}</NavLink>)}<p className="px-4 pb-1 pt-3 text-xs font-semibold uppercase tracking-wider text-slate-500">Upcoming</p>{upcomingLinks.map(([label, path]) => <NavLink key={path} to={path} onClick={() => setMenuOpen(false)} className={({ isActive }) => `block border-l-2 py-2 pl-7 text-sm ${isActive ? "border-emerald-300 text-white" : "border-transparent text-slate-400"}`}>{label}</NavLink>)}<p className="px-4 pb-1 pt-3 text-xs font-semibold uppercase tracking-wider text-slate-500">Developers</p>{developerLinks.map(([label, path]) => <NavLink key={path} to={path} onClick={() => setMenuOpen(false)} className="block py-2 pl-7 text-sm text-slate-400">{label}</NavLink>)}<p className="px-4 pb-1 pt-3 text-xs font-semibold uppercase tracking-wider text-slate-500">More</p>{moreLinks.map(([label, path]) => <NavLink key={path} to={path} onClick={() => setMenuOpen(false)} className="block py-2 pl-7 text-sm text-slate-400">{label}</NavLink>)}</nav>}</header>;
}