...
export default function AdminMetrics({ data }) {
  const metrics = [
    ...
    [Coins, "Rewards balance", sol(data.rewardsLamports), "Live on Solana"],
  ...
  return <div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">{metrics.map(([Icon, label, value, detail]) => <div key={label} className="card-glow"><Icon className="h-5 w-5 text-cyan-300" /><p className="mt-5 text-xs uppercase tracking-wider text-slate-500">{label}</p><p className="mt-1 text-2xl font-semibold text-white">{value}</p><p className="mt-1 text-xs text-slate-500">{detail}</p></div>)}</div>;
}