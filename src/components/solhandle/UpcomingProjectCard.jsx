...
export default function UpcomingProjectCard({ phase, title, status, summary, items, accent }) {
  ...
  return <article className={`flex h-full flex-col rounded-3xl border bg-slate-950/70 p-6 shadow-2xl backdrop-blur-xl md:p-8 ${color}`}>
    ...
    <div className="mt-7 flex items-center gap-2 border-t border-white/10 pt-5 text-sm font-medium text-white">Next project phase <ArrowRight className="h-4 w-4" /></div>
  </article>;
}