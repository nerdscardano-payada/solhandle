...
export default function RecentHandles() {
  ...
  return <section className="mt-12"><div className="mb-5 flex items-end justify-between"><div><p className="text-sm text-cyan-300">On-chain activity</p><h2 className="mt-1 text-2xl font-semibold text-white">Recently minted</h2></div><span className="text-xs text-slate-500">{showDemo ? "Demo artwork · not minted" : "Official SolHandle Core Assets"}</span></div>{handles === null ? <div className="card-glow text-slate-400">Loading recent mints…</div> : <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{cards.map(handle => <HandleNftCard key={handle.asset || handle.handle} handle={handle}/>)}</div>}</section>;
}