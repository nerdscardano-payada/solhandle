...
export default function RecentHandles() {
  ...
  return <section className="mt-12">
    ...
    {handles === null ? <div className="card-glow text-slate-400">Loading recent mints…</div> : <div className="grid grid-cols-2 gap-3 pb-3 lg:grid-cols-4 lg:gap-4">{handles.map((item) => <RecentHandleActivityCard key={item.asset || item.handle} item={item} />)}</div>}
  </section>;
}