...
export default function RecentHandles() {
  ...
  return <section className="mt-12">
    ...
    {handles === null ? <div className="card-glow text-slate-400">Loading recent mints…</div> : <div className="grid grid-cols-4 gap-2 pb-3 sm:gap-4">{handles.map((item) => <RecentHandleActivityCard key={item.asset || item.handle} item={item} />)}</div>}
  </section>;
}