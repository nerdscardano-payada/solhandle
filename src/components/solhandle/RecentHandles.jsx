...
export default function RecentHandles() {
  ...
  return <section className="mt-12">
    ...
    {handles === null ? <div className="card-glow text-slate-400">Loading recent mints…</div> : <div className="flex snap-x gap-4 overflow-x-auto pb-3 lg:grid lg:grid-cols-3 lg:overflow-visible">{handles.map((item) => <RecentHandleActivityCard key={item.asset || item.handle} item={item} />)}</div>}
  </section>;
}