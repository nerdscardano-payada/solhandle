...
export default function DiscordPublisher() {
  ...
  const publish = async (event) => {
    ...
    try {
      await base44.functions.invoke("publishDiscord", { type: "community", title, message });
      setTitle(""); setMessage(""); setStatus("Published to Discord.");
  ...
  return <form onSubmit={publish} className="card-glow mt-8">
    <p className="text-xs font-semibold uppercase tracking-wider text-violet-300">Discord community</p>
    <h2 className="mt-2 text-xl font-semibold text-white">Publish channel update</h2>
