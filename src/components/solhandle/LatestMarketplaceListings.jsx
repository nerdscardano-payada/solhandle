...
export default function LatestMarketplaceListings() {
  ...
  return <section className="rounded-xl border border-white/10 bg-[#11161f] p-4 shadow-xl shadow-black/20">
    ...
    {listings === null ? <div className="mt-4 h-20 animate-pulse rounded-lg bg-white/5"/> : listings.length ? <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">{listings.map((listing) => <Link key={listing.id} to={`/market?handle=${encodeURIComponent(listing.handle)}`} className="flex h-full flex-col rounded-xl border border-cyan-300/25 bg-slate-950/80 p-3 shadow-lg shadow-cyan-950/20 transition hover:-translate-y-0.5 hover:border-violet-400/50"><div className="mb-3 flex min-w-0 items-center justify-between gap-3"><span className="truncate text-lg font-semibold text-cyan-100">@{listing.handle}</span><span className="shrink-0 rounded-md bg-emerald-400/10 px-2 py-1 text-[10px] text-emerald-300">Core Asset ✓</span></div><HandleCard handle={listing.handle} className="rounded-xl border-white/5"/><div className="mt-3 flex items-end justify-between gap-3"><p className="text-[10px] uppercase tracking-wider text-slate-500">Native listing</p><p className="shrink-0 text-sm font-semibold text-emerald-300">{toSol(listing.price_lamports)} SOL</p></div></Link>)}</div> : <p className="mt-4 rounded-lg border border-dashed border-white/10 px-4 py-5 text-sm text-slate-400">No active marketplace listings yet.</p>}
  </section>;
}