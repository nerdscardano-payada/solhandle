...
export default function PayLinkGenerator({ handles, wallet, primaryHandle, loading }) {
  ...
  const url = handle && validAmount ? `${window.location.origin}/pay?handle=${encodeURIComponent(handle)}${amount ? `&amount=${encodeURIComponent(amount)}` : ''}` : '';
  useEffect(() => {
    let alive = true;
