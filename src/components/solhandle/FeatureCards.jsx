...
export default function FeatureCards({ wallet }) {
  ...
  useEffect(() => { let active=true; invokeWithRetry("getProtocolStats", {}).then(({data}) => { if(active){setStats(data);setStatsState("ready");} }).catch(() => active && setStatsState("error")); return () => { active=false; }; }, []);
