...
export default function PremiumDirectoryCard({ item }) {
  return <article className="rounded-2xl border border-white/10 bg-slate-950/70 p-5 shadow-lg shadow-black/20">
    ...
    <div className="mt-4 flex items-center justify-between gap-3 text-sm"><div className="flex items-center gap-2"><span className="text-emerald-300">Available</span>{item.premium && <span className="rounded-full border border-violet-300/40 bg-violet-300/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-violet-200">Premium</span>}</div><strong className="text-white">{lamportsToSol(item.priceLamports)} SOL</strong></div>
