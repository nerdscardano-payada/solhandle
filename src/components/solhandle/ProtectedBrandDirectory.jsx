...
export default function ProtectedBrandDirectory() {
  ...
  return <div><div className="mb-5 flex items-center justify-between"><p className="text-sm text-slate-400">{brands.length} protected {brands.length === 1 ? "organization" : "organizations"}</p><p className="flex items-center gap-2 text-xs text-emerald-300"><ShieldCheck className="h-4 w-4"/>Protected & reserved</p></div><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{brands.map((brand) => { const reserved = brand.restrictionType === "RESERVED"; return <article key={brand.handle} className="rounded-2xl border border-white/10 bg-slate-950/70 p-5 shadow-lg shadow-black/20"><div className="flex items-start justify-between gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl border border-cyan-300/20 bg-cyan-300/10"><Building2 className="h-5 w-5 text-cyan-200"/></div><span className="inline-flex items-center gap-1 rounded-full bg-emerald-300/10 px-2.5 py-1 text-xs text-emerald-200"><BadgeCheck className="h-3.5 w-3.5"/>{reserved ? "Reserved" : "Protected"}</span></div><h3 className="mt-5 text-xl font-semibold text-white">@{brand.handle}</h3>{brand.domain && <p className="mt-1 text-sm text-cyan-200">{brand.domain}</p>}<p className="mt-3 text-sm leading-relaxed text-slate-400">{brand.reason || (reserved ? "Reserved for the verified Solana organization." : "Protected against public minting and impersonation.")}</p><p className="mt-4 border-t border-white/5 pt-4 text-xs text-slate-500">{reserved ? "Eligible for a free official claim after verification and approval." : "Permanently unavailable for public or official claims."}</p></article>; })}</div></div>;
}