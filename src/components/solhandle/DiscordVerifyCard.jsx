...
export default function DiscordVerifyCard() {
  ...
  const { connected, verify, loading, error, handle } = useDiscordVerification(token);
  ...
  return <section className="card-glow max-w-xl">
    ...
    <div className="mt-6">{!connected ? <WalletMultiButton /> : <button type="button" disabled={!token || loading} onClick={verify} className="w-full rounded-lg bg-gradient-to-r from-cyan-300 to-violet-300 px-5 py-3 font-semibold text-slate-950 disabled:opacity-50">{loading ? "Verifying…" : "Sign and verify ownership"}</button>}</div>
