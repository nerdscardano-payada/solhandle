...
import { lamportsToSol, normalizeHandle, shortenAddress, validateHandle } from "@/lib/solhandle";
import { trackFunnel, trackProtocol } from "@/lib/protocolAnalytics";
...
import RushCountdown from "@/components/solhandle/RushCountdown";
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        if (active && res.data?.handle === handle) { setResult(res.data); base44.analytics.track({ eventName: "referral_handle_searched", properties: { available: Boolean(res.data.available) } }); trackProtocol("search", { handle, available: Boolean(res.data.available), status: res.data.status || (res.data.available ? "AVAILABLE" : "UNAVAILABLE") }); trackFunnel("SEARCH", handle); }
      } catch {
