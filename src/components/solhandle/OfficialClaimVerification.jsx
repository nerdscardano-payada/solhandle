...
export default function OfficialClaimVerification({ claim, step, onStepChange }) {
  ...
  if(step === 2) return <div className="space-y-4 text-sm"><div><h3 className="font-semibold text-white">Verify official domain</h3><p className="mt-1 text-slate-400">Publish the challenge below, then continue when DNS or the verification file is live.</p></div><div className="rounded-xl border border-cyan-300/20 bg-cyan-300/5 p-4"><p className="text-slate-300">Use DNS TXT at <b>_solhandle.{claim.domain}</b> or <b>https://{claim.domain}/.well-known/solhandle-verification.txt</b></p><div className="mt-3 flex items-center gap-2 rounded-lg bg-black/30 p-3"><code className="min-w-0 flex-1 break-all text-cyan-200">{claim.challenge}</code><button onClick={() => navigator.clipboard.writeText(claim.challenge)} aria-label="Copy challenge"><Copy className="h-4 w-4"/></button></div></div>{error && <p className="text-rose-300">{error}</p>}<button onClick={checkDomain} disabled={Boolean(busy)} className="w-full rounded-lg bg-gradient-to-r from-emerald-300 via-cyan-400 to-violet-500 py-3 font-semibold text-slate-950 disabled:opacity-60">{busy === "domain" ? "Checking domain…" : "Verify domain and continue"}</button></div>;
