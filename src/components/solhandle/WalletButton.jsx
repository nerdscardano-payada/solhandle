...
export default function WalletButton({ onConnected }) {
  ...
  async function connect() {
    ...
    setBusy(true);
    try { const result = await provider.connect(); const address = result.publicKey.toString(); localStorage.setItem("solhandle_wallet", address); setWallet(address); onConnected?.(address); } finally { setBusy(false); }
  }
