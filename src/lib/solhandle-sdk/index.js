...
function readU32(data, offset) {
  if (data.byteLength < offset + 4) throw new Error("Invalid SolHandle account data.");
  return new DataView(data.buffer, data.byteOffset, data.byteLength).getUint32(offset, true);
}

function decodeRecord(data) {
  const length = readU32(data, 8);
  if (length < 1 || length > 20 || data.byteLength < 44 + length) throw new Error("Invalid HandleRecord.");
  return { handle: decoder.decode(data.slice(12, 12 + length)), asset: new PublicKey(data.slice(12 + length, 44 + length)) };
}

function decodeAsset(data) {
  if (data.byteLength < 66 || data[0] !== 1) throw new Error("Invalid Metaplex Core asset.");
...
async function classifyDestination(connection, address, commitment) {
  ...
  const systemOwned = info.owner.equals(SystemProgram.programId);
  const safe = onCurve && systemOwned;
...
export async function resolveHandle(value, options = {}) {
  ...
  if (!validateHandle(handle)) throw new Error("Handle must use 1-20 lowercase letters or numbers.");
  const commitment = options.commitment || "confirmed";
  const connection = connectionFor(options);
  const handlePda = getHandlePda(handle);
  const assetPda = getAssetPda(handle);
  const [recordInfo, assetInfo] = await connection.getMultipleAccountsInfo([handlePda, assetPda], commitment);
  ...
  return {
    handle: `@${handle}`,
    address: asset.owner.toBase58(),
    status: "claimed",
    verified: true,
    collectionVerified: true,
    ...(await classifyDestination(connection, asset.owner, commitment)),
    handlePda: handlePda.toBase58(),
...
export async function reverseResolve(wallet, options = {}) {
  ...
  const commitment = options.commitment || "confirmed";
  const connection = connectionFor(options);
  const primaryPda = getPrimaryHandlePda(address);
  const info = await connection.getAccountInfo(primaryPda, commitment);
  if (!info?.owner.equals(PROGRAM_ID)) return null;
  const length = readU32(info.data, 8);
  if (length < 1 || length > 20 || info.data.byteLength < 44 + length) return null;
  const handle = decoder.decode(info.data.slice(12, 12 + length));
  const asset = new PublicKey(info.data.slice(12 + length, 44 + length));
  const resolved = await resolveHandle(handle, { connection, commitment });
...
export async function getHandlesByOwner(wallet, options = {}) {
  ...
  const commitment = options.commitment || "confirmed";
  const connection = connectionFor(options);
  const accounts = await connection.getProgramAccounts(PROGRAM_ID, { commitment });
  const records = [];
  for (const account of accounts) {
    try {
      const record = decodeRecord(account.account.data);
      if (validateHandle(record.handle) && getHandlePda(record.handle).equals(account.pubkey) && getAssetPda(record.handle).equals(record.asset)) records.push(record);
    } catch {
  ...
  for (let index = 0; index < records.length; index += 100) {
    ...
    assets.forEach((info, offset) => {
      ...
      if (asset.owner.equals(owner) && asset.collection?.equals(COLLECTION_ID)) matches.push(`@${batch[offset].handle}`);
    });
  }
  return matches;
}