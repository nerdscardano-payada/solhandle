...
// Self-contained: no filters, no <use> refs — only inline elements + universal
// gradients so it also rasterizes cleanly via <canvas> on the frontend.
...
export function buildHandleCardSvg(handle) {
  const label = `@${handle}`;
