...
// (wave background + gradient-bordered dark card + Solana logomark + gradient handle
// text + SOLHANDLE label + Official badge). Output is a self-contained 16:9 SVG string
// that renders identically in any wallet or marketplace preview. Uses the official
// Solana logomark path data (three parallelograms, viewBox 0 0 397 397).
...
export function buildHandleCardSvg(handle: string): string {
  ...
  const logoSize = 397 * logoScale;
  const logoX = 640 - logoSize / 2;
  const logoY = 150;
...
<svg xmlns="http://www.w3.org/2000/svg" width="1280" height="720" viewBox="0 0 1280 720">
  <defs>
    ...
    <linearGradient id="solMark" x1="18" y1="125" x2="379" y2="272" gradientUnits="userSpaceOnUse">
  ...
  <g filter="url(#borderGlow)">
    <rect x="280" y="96" width="720" height="528" rx="40" fill="#04060D" stroke="url(#borderGrad)" stroke-width="3"/>
  </g>

  <use href="#solanaLogo" transform="translate(${logoX.toFixed(0)} ${logoY.toFixed(0)}) scale(${logoScale})"/>

  <text x="${textX.toFixed(0)}" y="420" font-family="Arial, Helvetica, sans-serif" font-weight="700" font-size="${fontSize}" text-anchor="middle" fill="url(#handleGrad)" filter="url(#textGlow)">${escapeXml(label)}</text>
