...
async function ensurePromoterProfile(base44, wallet, handle) {
...
async function reconcileProfile(base44, profile) {
...
export async function processConfirmedReferral(base44, mint) {
  ...
  if (existing[0]) {
    const profiles = await base44.asServiceRole.entities.ReferralProfile.filter({ id: existing[0].referral_profile_id }, "-created_date", 1);
    if (profiles[0] && existing[0].status === "APPROVED") {
      const ledger = await base44.asServiceRole.entities.ReferralLedger.filter({ conversion_id: existing[0].id }, "-created_date", 1);
      if (!ledger[0]) await base44.asServiceRole.entities.ReferralLedger.create({ referral_profile_id: profiles[0].id, type: "REFERRAL_REWARD", amount_lamports: existing[0].reward_amount_lamports, conversion_id: existing[0].id, payout_id: "", status: "PENDING" });
      await reconcileProfile(base44, profiles[0]);
    }
  ...
  if (!intent?.referral_profile_id || intent.status !== "CONFIRMED") return { credited: false, reason: "no_referral" };
  ...
  if (!profile || profile.status !== "ACTIVE") return { credited: false, reason: "inactive_profile" };
  ...
  const percentage = tierPercentage(settings, profile.successful_referrals + 1);
  const selfReferral = profile.wallet_address === mint.buyerWallet;
  const conversion = await base44.asServiceRole.entities.ReferralConversion.create({
    ...
    buyer_wallet: mint.buyerWallet, referral_profile_id: profile.id, gross_mint_amount_lamports: mint.grossAmountLamports,
    eligible_referral_revenue_lamports: eligible, reward_percentage_used: percentage,
    reward_amount_lamports: selfReferral ? 0 : Math.floor(eligible * percentage / 100),
    status: selfReferral ? "REJECTED_SELF_REFERRAL" : "APPROVED"
  });
  if (selfReferral) {
  ...
  await reconcileProfile(base44, profile);
  return { credited: true, rewardLamports: conversion.reward_amount_lamports, percentage };
}