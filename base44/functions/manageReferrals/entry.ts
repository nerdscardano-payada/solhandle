...
export default async function(req: Request): Promise<Response> {
  try {
    ...
    if (action === "flag_status") { if (!["OPEN", "CLEARED", "BLOCKED"].includes(body.status)) return Response.json({ error: "Invalid flag status." }, { status: 400 }); await base44.asServiceRole.entities.FraudFlag.update(String(body.flag_id), { status: body.status }); return Response.json({ updated: true }); }
    if (action === "bonus") { const amount = Math.floor(Number(body.amount_sol) * 1e9); if (!(amount > 0)) return Response.json({ error: "Invalid bonus." }, { status: 400 }); const profiles = await base44.asServiceRole.entities.ReferralProfile.filter({ id: String(body.profile_id) }, "-created_date", 1); if (!profiles[0]) return Response.json({ error: "Profile not found." }, { status: 404 }); await base44.asServiceRole.entities.ReferralLedger.create({ referral_profile_id: profiles[0].id, type: "BONUS", amount_lamports: amount, conversion_id: "", payout_id: "", status: "AVAILABLE" }); await reconcileReferralProfile(base44, profiles[0]); return Response.json({ updated: true }); }
