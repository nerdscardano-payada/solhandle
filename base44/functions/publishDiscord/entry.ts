  ...
  try {
    ...
    if (body.type === "registration") {
      ...
      payload = {
        username: "SolHandle",
        embeds: [{
          title: `New handle registered: ${handleRecord.display_handle || `@${handleRecord.handle}`}`,
          description: "A new human-readable identity is now live on Solana.",
          color: 4514520,
          fields: [
            { name: "Owner", value: short(handleRecord.current_owner_cached || handleRecord.original_minter), inline: true },
            { name: "Rarity", value: handleRecord.rarity?.replaceAll("_", " ") || "STANDARD", inline: true },
            { name: "Asset", value: handleRecord.asset_address ? `[View on Solana Explorer](${explorerUrl})` : "Indexing", inline: false }
          ],
      ...
      const title = String(body.title || "Community update").trim().slice(0, 100);
      const message = String(body.message || "").trim().slice(0, 1800);
      if (!message) return Response.json({ error: "Message is required." }, { status: 400 });
      payload = { username: "SolHandle", embeds: [{ title, description: message, color: 6407404, timestamp: new Date().toISOString() }] };
    } else {
    ...
    return Response.json({ error: error.message || "Discord publication failed." }, { status: 500 });
  }
}