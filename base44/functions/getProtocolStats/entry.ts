...
export default async function(req: Request): Promise<Response> {
  try {
    ...
    const [latest, activeHandles, transactions] = await Promise.all([
    ...
    return Response.json({ totalMinted: config?.total_minted ?? activeHandles.length, uniqueHolders, minted24h: recentHandles.length, mintVolume24hSol, scarcity, rarityDistribution, collection: config?.collection ?? null, paused: config?.paused ?? null, lastSync: config?.last_sync ?? null });
