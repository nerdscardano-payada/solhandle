...
export default async function(req: Request): Promise<Response> {
  try {
    ...
    if (!marketResponse.ok) throw new Error('Token price feed unavailable; snapshot was not updated.');
    ...
    let cycle = cycles[0];
    if (!cycle) cycle = await base44.asServiceRole.entities.GrowthCycle.create({ cycle_number: 1, started_at: stamp, handles_start: minted, holders_start: eligible.length, handles_target: TARGETS[0], holders_target: TARGETS[0], price_start_usd: price, price_now_usd: price, price_target_percent: PRICE_TARGET_PERCENT, handles_now: minted, holders_now: qualified, max_progress: 0, last_checked_at: stamp });
    else {
      const changes = {};
      if (cycle.cycle_number === 1 && cycle.handles_target === 250) { changes.handles_target = TARGETS[0]; changes.holders_target = TARGETS[0]; }
      ...
      if (!cycle.price_start_usd) { changes.price_start_usd = price; changes.price_target_percent = PRICE_TARGET_PERCENT; }
      ...
      if (handleShare === 1 && holderShare === 1 && priceShare === 1) {
        await base44.asServiceRole.entities.GrowthCycle.update(cycle.id, { handles_now: minted, holders_now: qualified, price_now_usd: price, max_progress: 100, last_checked_at: stamp });
        cycle = await base44.asServiceRole.entities.GrowthCycle.create({ cycle_number: cycle.cycle_number + 1, started_at: stamp, handles_start: minted, holders_start: qualified, handles_target: TARGETS[Math.min(cycle.cycle_number, TARGETS.length - 1)], holders_target: TARGETS[Math.min(cycle.cycle_number, TARGETS.length - 1)], price_start_usd: price, price_now_usd: price, price_target_percent: PRICE_TARGET_PERCENT, handles_now: minted, holders_now: qualified, max_progress: 0, last_checked_at: stamp });
      } else cycle = await base44.asServiceRole.entities.GrowthCycle.update(cycle.id, { handles_now: minted, holders_now: qualified, price_now_usd: price, max_progress: maxProgress, last_checked_at: stamp });
    }
